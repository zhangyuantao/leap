...
class Camera {
	...
	private followTarget() {
		...
		// 目标距离世界中心距离
		let disToCenter = Math.sqrt(self.target.x * self.target.x + self.target.y * self.target.y);

		// 根据目标距离圆心的距离来缩放“世界”
		let toScale = 1 - disToCenter / self.worldRadius;
		toScale = Math.max(toScale, self.worldMinScale);
		self.worldRoot.scaleX = parseFloat(toScale.toFixed(3));
		self.worldRoot.scaleY = parseFloat(toScale.toFixed(3));
		...
		let worldOffsetX = (self.targetMoveRadius * cos) | 0;
		let worldOffsetY = (self.targetMoveRadius * sin) | 0;
		...
		self.worldRoot.x = parseFloat(x.toFixed(2));
		self.worldRoot.y = parseFloat(y.toFixed(2));
	}
}